...
case class Log(ct: Long, level: LogLevel, text: String, timestamp: Long)
...
object Log {
  ...
  private val loggerLock = new Object
  private val loggerCounter = new AtomicLong()
  private val logger = ReplaySubject[Log](1000 * 1000)

  def logsObservable: Observable[Log] = logger.subscribeOn(NewThreadScheduler())
  ...
  def log(level: LogLevel, message: => String = ""): Log.type = {
    ...
    def logText = stackClassAndMethod + (if (messageText != "") {
      ": " + messageText
    } else "")

    if (actualLevel.shouldLog(level)) {
      val text = logText
      loggerLock.synchronized {
        val ct = loggerCounter.incrementAndGet()
        val l = Log(ct, level, text, System.currentTimeMillis)
        logger.onNext(l)
      }
    }
    this
  ...
  val logCounter2 = new AtomicLong()
  logsObservable.subscribe(
    log => {
      val observerCounter = logCounter2.incrementAndGet()
      if (log == null) s"log was null".printlnErr
      else if (observerCounter != log.ct) ("DIFFERENCE of logger and observer counter ObserverCt:" + observerCounter + ", logCt: " + log.ct + ", Log: " + logToConsoleText(log)).printlnErr
    ...
    throwable => {
      ("ERROR HAPPENED IN LOGGING " + throwable).printlnErr
...
abstract class LogLevel(val logPriority: Int) {
  def shouldLog(otherLevel: LogLevel) = otherLevel.logPriority <= logPriority

  def <(otherLevel: LogLevel) = otherLevel.logPriority > logPriority
