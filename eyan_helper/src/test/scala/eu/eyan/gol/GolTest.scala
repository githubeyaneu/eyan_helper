...
case class Gol(aliveCells: Set[Cell]) {
  def nextGeneration = Gol(aliveCellsWithTwoAliveNeighbours ++ cellsAndCandidatesWithThreeNeighbours)

  private def aliveCellsWithTwoAliveNeighbours = aliveCells filter aliveNeighboursCount(2)
  private def cellsAndCandidatesWithThreeNeighbours = cellsWithAllNeighbours filter aliveNeighboursCount(3)
