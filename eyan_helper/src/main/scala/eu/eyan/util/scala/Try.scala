...
object FinallyClose { def apply[T, CLOSEABLE <: Closeable](closeable: CLOSEABLE, action: CLOSEABLE => T): T = TryCatchFinally(action(closeable), e => { Log.error(e); null.asInstanceOf[T] }, CloseablePlus.closeQuietly(closeable)) }
